...
def dp_dns(token_id, token_key, domain, record, ip):
    ...
    global_params = {"login_token": "%d,%s" % (token_id, token_key), "format": "json"}
    headers = {
    ...
    records = requests.post(
        api_url + "Record.List", data=urlencode(global_params), headers=headers
    ).json()
    record_id = [i["id"] for i in records["records"] if i["name"] == record][0]
    global_params.update(
        {"record_id": record_id, "sub_domain": record, "record_line_id": 0, "value": ip}
    )
    result = requests.post(
        api_url + "Record.Ddns", data=urlencode(global_params), headers=headers
    )
...
def cf_dns(token_id, token_key, domain, record, ip):
    ...
    headers = {
        "Authorization": "Bearer %s" % token_key,
    ...
    records = requests.get(
        api_url + "zones/%s/dns_records" % domain_id, params=params, headers=headers
    ).json()
    record_id = records["result"][0]["id"]

    data = {"type": "A", "name": record, "content": ip}
    result = requests.put(
        api_url + "zones/%s/dns_records/%s" % (domain_id, record_id),
        json=data,
        headers=headers,
    )
    return result.status_code == 200
